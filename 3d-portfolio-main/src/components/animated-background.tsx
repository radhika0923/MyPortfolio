...
import { KEYBOARD_SKILLS, Skill, SkillNames, SKILLS } from "@/data/constants";
...
const AnimatedBackground = () => {
  ...
  const getKeycapsAnimation = () => {
    ...
    const start = () => {
      removePrevTweens();
      KEYBOARD_SKILLS.map((skillName) => SKILLS[skillName])
    ...
    const stop = () => {
      removePrevTweens();
      KEYBOARD_SKILLS.map((skillName) => SKILLS[skillName]).forEach((skill) => {
        const keycap = splineApp.findObjectByName(skill.name);
        if (!keycap) return;
        const t = gsap.to(keycap.position, {
          y: 0,
